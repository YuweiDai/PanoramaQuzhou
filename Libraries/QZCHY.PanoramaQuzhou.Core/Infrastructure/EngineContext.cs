        ...
        protected static IEngine CreateEngineInstance(PanoramaQuzhouConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'QZCHY.PanoramaQuzhou.Core.Infrastructure.IEngine' and cannot be configured in /configuration/QMTech/engine[@engineType] for that purpose.");
